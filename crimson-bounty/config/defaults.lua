--- The configuration as this resource ships it.
---
--- NOT the file to edit. Edit config/config.lua; this one exists so that a
--- setting added to the resource after an operator last touched their copy
--- still has a value.
---
--- config.lua is a file operators edit, so it stops tracking the shipped one
--- the moment they do. Every setting added afterwards is simply absent from
--- their copy, and absent reads as nil — which the server then indexes,
--- inside a request handler, where it becomes a crash a player reads as
--- "Could not read what you are carrying" and an operator sees as nothing at
--- all. That has happened three times on live servers.
---
--- Boot fills any section this has and config.lua does not. It fills WHOLE
--- MISSING SECTIONS only: a table the operator has written is theirs, and
--- topping it up would silently put back entries they deliberately removed —
--- BlockedJobNames is a set, and restoring a job somebody took out of it
--- would be worse than the gap. Per-key filling is deliberate, and lives in
--- server/main.lua's DEFAULTS for the handful of record-shaped settings
--- where it is safe.
---
--- Keep this in step with config.lua when a setting is added. The static
--- check fails the build if a section exists in one and not the other.

ConfigDefaults = ConfigDefaults or {}
...
ConfigDefaults.BlockedJobTypes = {
...
ConfigDefaults.BlockOffDuty = true
...
ConfigDefaults.HideAppFromBlockedJobs = true
...
ConfigDefaults.MaxContractValue = 1000000

--- Items that may never be escrowed, whatever the creator selects.
ConfigDefaults.EscrowBlacklist = {
...
ConfigDefaults.Listing = {
    PageSize = 15,
}

ConfigDefaults.Targeting = {
